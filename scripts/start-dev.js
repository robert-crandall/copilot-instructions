...
const PID_FILE = path.join(os.tmpdir(), "example-app-dev.pid");
...
async function isFrontendRunning() {
  return await isServiceRunning(FRONTEND_PORT, [
    "svelte",
    "SvelteKit",
    "Life Quest",
  ]);
  ...
  ports.forEach((port) => {
    ...
    if (pids.length > 0) {
      try {
        ...
        log(
          `Killed process(es) on port ${port}: ${pids.join(", ")}`,
          "success",
        );
      } catch (e) {
...
function killDevProcesses() {
  try {
    ...
    const frontendResult = spawnSync("pgrep", ["-f", "vite dev"], {
      encoding: "utf-8",
    });
...
async function startBackend() {
  ...
  while (attempts < maxAttempts) {
    ...
    if (await isBackendRunning()) {
      log(
        `Backend started successfully on http://localhost:${BACKEND_PORT}`,
        "success",
      );
      return backend;
    }
...
async function startFrontend() {
  ...
  while (attempts < maxAttempts) {
    ...
    if (await isFrontendRunning()) {
      log(
        `Frontend started successfully on http://localhost:${FRONTEND_PORT}`,
        "success",
      );
      return frontend;
    }
...
async function startDevelopment() {
  try {
    log("Starting Example App Development Environment", "start");
    ...
    if (isForce || isStop) {
      ...
      if (!isFrontendOnly || isStop) {
        ...
        if (backendPids.length > 0) {
          spawnSync("kill", ["-9", ...backendPids]);
          log(
            `Killed backend process(es): ${backendPids.join(", ")}`,
            "success",
          );
        }
      ...
      if (!isBackendOnly || isStop) {
        ...
        if (frontendPids.length > 0) {
          spawnSync("kill", ["-9", ...frontendPids]);
          log(
            `Killed frontend process(es): ${frontendPids.join(", ")}`,
            "success",
          );
        }
    ...
    if (!isForce) {
      ...
      if (!isFrontendOnly && backendRunning) {
        log(
          `Backend already running on http://localhost:${BACKEND_PORT}`,
          "success",
        );
        if (isBackendOnly) {
      ...
      if (
        ...
        log(
          "Both services already running. Use --force to restart.",
          "success",
        );
        return;
      }
    ...
    if (!isForce) {
      if (
        !isFrontendOnly &&
        !backendRunning &&
        getPidsOnPort(BACKEND_PORT).length > 0
      ) {
        log(
          `Port ${BACKEND_PORT} is in use but backend is not responding. Use --force to kill and restart.`,
          "error",
        );
        process.exit(1);
      }
      ...
      if (
        !isBackendOnly &&
        !frontendRunning &&
        getPidsOnPort(FRONTEND_PORT).length > 0
      ) {
        log(
          `Port ${FRONTEND_PORT} is in use but frontend is not responding. Use --force to kill and restart.`,
          "error",
        );
        process.exit(1);
      }
    ...
    if (processes.length > 0) {
      ...
      if (isBackground) {
        log(
          'Services are running in background mode. Use "kill $(cat ' +
            PID_FILE +
            ')" to stop them.',
          "info",
        );
        ...
        await Promise.all(
          processes.map(
            (process) => new Promise((resolve) => process.on("exit", resolve)),
          ),
        );
      }
    }
